(ns design.operation
  "OperationActor -- one design operation = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor (Designer-LLM)
  is sealed into a single node (:advise); its proposal is ALWAYS
  routed through the Design Delivery Governor (:govern) and the
  ...
  One graph run = one design operation (intake -> advise -> govern ->
  ...
  decision to a human operator (studio operator). The approver resumes
  with `{:approval {:status :approved}}` (or :rejected). `:actuation/
  release-deliverable` ALWAYS reaches this node when the governor is
  clean -- see `design.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [design.designadvisor :as designadvisor]
...
(defn build
  ...
  `design.store/Store`).
  opts:
    :advisor      -- a `design.designadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (designadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Designer-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (designadvisor/-advise advisor store request)]
            {:proposal p :audit [(designadvisor/trace request p)]})))

      ;; Design Delivery Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
